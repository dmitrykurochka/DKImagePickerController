...
class DKAssetGroupListVC: UITableViewController, DKGroupDataManagerObserver {
	...
	convenience init(selectedGroupDidChangeBlock: @escaping (_ groupId: String?) -> Void, defaultAssetGroup: PHAssetCollectionSubtype?) {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if assetGroup.totalCount == 0 {
			...
			getImageManager().groupDataManager.fetchGroupThumbnailForGroup(assetGroup.groupId,
				...
				options: self.groupThumbnailRequestOptions) { image, _ in
				if cell.tag == tag {
